...
public class MainActivityService {
    ...
    public synchronized void setState(int state) {
        mState = state;
        mHandler.sendMessage(Message.obtain(mHandler,SecondActivity.MESSAGE_STATE_CHANGE,state,-1));
    }
    ...
    private synchronized void manageConnectedSocket(BluetoothSocket socket, BluetoothDevice device) {
        ...
        // 可以将连接的信息传递到主线程
        Message message = Message.obtain(mHandler, SecondActivity.MESSAGE_DEVICE_NAME);
        Bundle bundle = new Bundle();
        bundle.putString(SecondActivity.DEVICE_NAME, device.getName());
    ...
    private void connectionFailed() {
        ...
        // 连接失败了，UI上展示
        Message message = Message.obtain(mHandler, SecondActivity.MESSAGE_TOAST);
        Bundle bundle = new Bundle();
        bundle.putString(SecondActivity.TOAST, "不能连接到设备");
        message.setData(bundle);
        mHandler.sendMessage(message);
    ...
    private void connectionLost() {
        ...
        Message message = Message.obtain(mHandler, SecondActivity.MESSAGE_TOAST);
        Bundle bundle = new Bundle();
        bundle.putString(SecondActivity.TOAST, "连接到设备中断");
        message.setData(bundle);
        mHandler.sendMessage(message);
    ...
    private class ConnectedThread extends Thread {
        ...
        public void run() {
            ...
            while (true) {
                try {
                    ...
                    // 拿到消息之后可以在UI上有所展示
                    mHandler.sendMessage(Message.obtain(mHandler, SecondActivity.MESSAGE_READ, bytes, -1, buffer));
        ...
        public void write(byte[] bytes) {
            try {
                ...
                // 在UI上有所展示
                mHandler.sendMessage(Message.obtain(mHandler, SecondActivity.MESSAGE_WRITE, -1, -1, bytes));
            } catch (IOException e) {
            }
        }
